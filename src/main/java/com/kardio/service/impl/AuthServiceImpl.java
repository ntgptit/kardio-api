...
public class AuthServiceImpl implements AuthService {
	...
	public UserResponse register(RegisterRequest request) {
		...
		User user = User.builder().email(request.getEmail()).password(passwordEncoder.encode(request.getPassword()))
				.firstName(request.getFirstName()).lastName(request.getLastName()).active(true).build();
	...
	public AuthResponse authenticate(AuthRequest request) {
		...
		try {
			...
			return AuthResponse.builder().token(token).user(userMapper.toUserResponse(user)).build();
